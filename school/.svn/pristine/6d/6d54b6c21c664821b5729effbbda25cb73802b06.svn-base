...
Ext.define("School.controller.multimedia.AlbumMgr", {
	...
	init: function(application) {
		...
		this.control({
			...
			//编辑相册
			'albummgr button#editAlbum': {
				click: function(button) {
					this.updateAlbumName(button);						
				}
			},
	...
	updateAlbumName: function(button) {
		var grid = button.up("gridpanel"), 
			record = grid.getSelectionModel().getSelection(),
			store = grid.getStore(), 
			requestUrl = zyHost + "album/updateAlbumName.action ";

		//判断是否选中了一个相册
		if(record.length === 0) {
			School.util.Util.showErrorMsg("请选择一个需要修改的相册");
			return ;
		}

		var name = record[0].get('name');
		var albumId = record[0].get("id");
		...
		Ext.create('Ext.window.Window', {
			...
			buttons: [{
				...
				handler: function(button) {
					var name = button.up('window').down('textfield#newAlbumName').getValue();
					if(name == null ||name ==""){
						School.util.Util.showErrorMsg("相册名字不能为空！");
						return;
					}
					Ext.Ajax.request({
						url: requestUrl,
						...
						success: function() {
							...
							School.util.Util.showSuccessMsg('修改相册成功！');										
							store.reload();
						}
					});								
				}
