...
public class StudentServiceImpl implements StudentService {
	...
	public boolean uploadStudentAndParentInfo(String fileName, String realPath,
			String clazzId,List<Student> students) {
		// 1.声明变量
		List<String> headList = createHeadList();
		Map<String, String> map = createHeadListMap();
		ExcelUtil poi = new ExcelUtil();
		...
		for (StudentAndParentInfo info : result) {
			if (!NotEmptyString.isNotEmpty(new String[]{info.getStudentName(),
					info.getStudentIDCard(), info.getParentPhone()})) {
				continue;
			}
			//判断学生是否重复
			Student oldStudent = this.judgeStudent(info.getStudentName(), info.getParentPhone());
			if(null != oldStudent) {
				oldStudent.setName(info.getStudentName());
			...
			if (student == null) {
				...
				if(NotEmptyString.isNotNullString(info.getFeteday())) {
			...
			if (parent1 == null) {
				...
				// 家长姓名可为空
				if (NotEmptyString.isNotNullString(info.getParentName())) {
			...
			Parent parent2 = null;
			if(NotEmptyString.isNotNullString(info.getParent2Phone())) {
				parent2 = parentMapper.findByMoble(info.getParent2Phone());
				if (parent2 == null) { // 只有当副账号尚未注册 且用户 要注册两个账号时执行
					parent2 = new Parent();
					parent2.setId(UUIDUtil.getUUID());
					parent2.setCreateTime(new Date());
					if (NotEmptyString.isNotNullString(info.getParent2Name())) {
	...
	public boolean deleteStudentAndParent(String studentId, String parentId) {
		try {
				...
				//学生分数表
				boolean flag = this.scoreMapper.deleteScoreByStudentId(studentId);
				
				//学生评价表
			
				flag = this.appraiseMapper.deleteAppraiseByStudent(studentId);
				
				//学生请假表
				flag = this.leaveMapper.deleteLeaveByStudentId(studentId);
			
				//学生健康表
				flag = this.healthyMapper.deleteHealthyByStudentId(studentId);
				
				flag = studentMapper.deleteByPrimaryKey(studentId)>0;
				return flag;
		} catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException();
	...
	public List<Student> getAllStudentBySchool(String schoolId) {
		return this.studentMapper.getAllStudentBySchool(schoolId);
	}

	@Override
	public List<Student> getAllStudentByGradeId(String gradeIds) {
		List<Student> list = new ArrayList<Student>();
		String[] gradeIdList = gradeIds.split("\\.");
		for(String gradeId: gradeIdList) {
			list.addAll(this.studentMapper.getAllStudentByGradeId(gradeId));
		}
		return list;
	}
	...
	public Student judgeStudent(String studentName, String parentPhone) {
		...
		List<Student> studentLists = studentMapper.findByParentIdAndName(params);
		if(null != studentLists && studentLists.size()>0) {
			Student student = studentLists.get(0);
			params.put("studentId", student.getId());
			params.put("classId", student.gettScClassId());
			String schoolName = studentMapper.findSchoolNameByStudent(params);
