...
@Service
public class ClazzPhotoUpdateProcess implements ClazzPhotoProcess {
	private static final Logger logger = Logger
			.getLogger(ClazzPhotoUpdateProcess.class);
	@Autowired
	...
	public boolean doProcess(ClazzPhotoProcessContext context) {
		try {
			PhotoInfoResult result = photoService.photoUpdate(context.getInfo());
