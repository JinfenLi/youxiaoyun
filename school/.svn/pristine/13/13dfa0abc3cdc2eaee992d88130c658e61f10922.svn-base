...
public class CurriculaController {
	...
	@ResponseBody
	public String createCurriculas(String[] gradesName, String[] terms, Curricula curricula) {
		...
		for (; i < gradesName.length; i++) {
			for(int j = 0;j<terms.length;j++){
					...
					curricula.setAdaptiveTerm(terms[j]);
	...
	public String chooseCurricula(String subjectId, String adaptiveGrade,
		...
		ChooseCurriculaInfo chooseCurriculaInfo = null;
		...
		resultMap.clear();
		if(curriculas.size()==1){
			for(ClazzInfo clazz : infos){
				params.put("classId", clazz.getId());
				params.put("curriculaId", curriculas.get(0).getId());
				clazz.setCurriculaId(curriculas.get(0).getId());
				clazz.setCurriculaName(curriculas.get(0).getName());
				chooseCurriculaInfo = this.curriculaVariableService.getClazzCurriculaInfo(params);
				if(chooseCurriculaInfo != null){
					clazz.setCurriculaTeacherId(chooseCurriculaInfo.getTeacherId());
					clazz.setCurriculaTeacherName(chooseCurriculaInfo.getTeacherName());
				}
				params.clear();
			}
		}
	 ...
	 */
	@Transactional
	@RequestMapping("/updateCurriculaInfo")
	@ResponseBody
	public String updateCurriculaVariable(CurriculaVariable curriculaVariable){
		Map<String, Object> resultMap = new HashMap<String, Object>();
		resultMap.put("success", false);
		if(NotEmptyString.isNotEmpty(new String[]{
				curriculaVariable.getId(),
				curriculaVariable.gettScClassId(),
				curriculaVariable.gettScCurriculaId(),
				curriculaVariable.gettScSemesterId(),
				curriculaVariable.gettScTeacherId()
		})){
				try{
					CurriculaVariable oldCurriculaVariable = this.curriculaVariableService.selectByPrimaryKey(curriculaVariable.getId());
					oldCurriculaVariable.settScClassId(curriculaVariable.gettScClassId());
					oldCurriculaVariable.settScCurriculaId(curriculaVariable.gettScCurriculaId());
					oldCurriculaVariable.settScSemesterId(curriculaVariable.gettScSemesterId());
					oldCurriculaVariable.settScTeacherId(curriculaVariable.gettScTeacherId());
					System.out.println(oldCurriculaVariable.gettScClassId()+"classId");
					System.out.println(oldCurriculaVariable.gettScCurriculaId()+"CURRiD");
					System.out.println(oldCurriculaVariable.getId()+"id");
					System.out.println(oldCurriculaVariable.gettScSemesterId()+"semeId");
					System.out.println(oldCurriculaVariable.gettScTeacherId()+"teaId");
					System.out.println(oldCurriculaVariable.getCreateTime());
					if(this.curriculaVariableService.updateByPrimaryKey(oldCurriculaVariable)){
						resultMap.put("success", true);
					}
				}
				catch(Exception e){
	...
	public String deleteCurricualVariable(String curriculaVariableId) {
		...
		if (!flag) {
			...
			resulMap.put("msg", "该选课已有考试，请先删除该选课的考试再删除该选课");
		}
	...
	public String showCurriculaVariable(String gradeId, String subjectId,
			String teacherId, String semesterId, String classId, Integer start, Integer limit, Integer showTrunk) {

			//设定是否显示主干课程的阈值，若不传则默认为1
			if(showTrunk==null){
				showTrunk = 1 ;
			}
		System.out.println(showTrunk);
		Map<String, Object> resultMap = new HashMap<String, Object>();
		if (!NotEmptyString.isAtLeastOneNotEmpty(new String[] { gradeId,
				subjectId, teacherId, semesterId, classId })) {
			resultMap.put("success", true);
		...
		List<CurriculaVariableInfo2> info = curriculaVariableService
				.showCurriculaVariable(subjectId, gradeId, teacherId,
						semesterId, classId, start, limit, showTrunk); // 查询选课情况
		resultMap.put("totalCount", curriculaVariableService.selectCount(
				subjectId, gradeId, teacherId, semesterId, classId, showTrunk)); // 查询记录数
		resultMap.put("success", true);
	...
	@RequestMapping("/getCurriculaInfo")
	@ResponseBody
	public String getCurriculaInfoByClassIdAndSemesterId(String semesterId,String classId){
		HashMap<String, Object> resultMap=new HashMap<String, Object>();
		resultMap.put("success", false);
		if(NotEmptyString.isNotNullString(semesterId)&&NotEmptyString.isNotNullString(classId)){
				List<CurriculaInfoVo> curriculaInfo = new ArrayList<CurriculaInfoVo>();
				curriculaInfo=this.curriculaService.getCurriculaInfo(semesterId, classId);
				resultMap.put("curriculaInfo", curriculaInfo);
				resultMap.put("success", true);
		}
		return JSONUtil.toObject(resultMap).toString();
	}
