...
import com.topview.school.service.user.ShowFeaturesService;
...
import com.topview.school.vo.User.UserLoginInfoList;
...
public class UserController {
	...
	@Resource
	private ShowFeaturesService showFeaturesService;
	@Resource
	...
	public @ResponseBody Object login(@Valid LoginRequest loginRequest, Model model, HttpSession session,
			String student_id) {
		...
		String[] filter = { "createTime", "lastupdate", "password", "kids", "tag", "info", "sortName", "comment",
				"teaching_classes" };
	...
	public @ResponseBody String getOfflineMessage(HttpSession session) {
		...
		if (userInfo != null) {
			List<OfflineMessageVo> vos = pushMsgService.getOfflineMessage(userInfo.getUser_id()).getResult(); // 查询该用户未接收信息
			if (vos != null && !"".equals(vos)) {
				for (OfflineMessageVo vo : vos) {
					if (pushMsgService.pushMessage(vo).isSuccess()) { // 推送
						if (!"我的短信".equals(vo.getType())) {
							pushService.updateMessage(vo.getEnvelopeId()); // 修改发送状态，TODO
																			// if块看上去可以不写，这里为什么要加上
						}
					}
				}
	...
	public String getUserInfo(HttpSession session) {
		...
		SemesterVo semesterVo = semesterService.getCurrentSemester(userInfo.getSchool_id()); // 当前学期

		if (userType == UserType.Teacher) { // 教师类型
			String[] filter = { "createTime", "password", "superPassword", "tScClassId", "birthday", "education",
					"lastUpdate", "tScGradeId", "tScSchoolId" };
			...
			List<Clazz> clazzs = clazzService.selectTeacherClazzs(userId, semesterVo.getId()); // 查询当前学期所带的所有班级
			...
			List<TeacherPosition> positions = teacherPositionService.getPositions(userId); // 查询职务
			if (positions.size() > 0) {
				List<String> list = new ArrayList<String>();
			...
			String[] filter = { "createTime", "parentInfo.idcard", "parenttype", "password", "superPassword", "type",
					"tScClassId" };
	...
	@ResponseBody
	public String updateUserInfo(HttpSession session, StudentAndParentInfo studentAndParentInfo, TeacherVo teacherInfo)
			throws ParseException {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		resultMap.put("success", false);
		...
		if (userInfo.getUser_type().ordinal() == 1) { // 1为教师类型
			...
			flag = teacherService.updateTeacherInfo(teacher);
			if(flag) {
				sendContact(userInfo.getSchool_id(),userInfo.getUser_id());
			}
			resultMap.put("success", flag);
		} else {// TODO此处逻辑有错待改正
			if (studentAndParentInfo.getParentId() != null && !"".equals(studentAndParentInfo.getParentId())) {
				Parent p = new Parent();
				...
				flag = parentService.updateParent(p);
				//resultMap.put("success", parentService.updateParent(p));
			}
			if (studentAndParentInfo.getStudentId() != null && !"".equals(studentAndParentInfo.getStudentId())) {
				Student s = new Student();
				s.setId(studentAndParentInfo.getStudentId());
				s.setAddress(studentAndParentInfo.getStudentAddress());
				if (studentAndParentInfo.getFeteday() != null && !"".equals(studentAndParentInfo.getFeteday())) {
					s.setBirthday(DateFormatUtil.parseToDay(studentAndParentInfo.getFeteday()));
				}
				...
				flag1 = studentService.updateStudent(s);
			...
			if(flag && flag1) {
				...
				sendContact(temp.gettScClassId(),userInfo.getUser_id());
				resultMap.put("success", true);
			} else {
				resultMap.put("success",false);
			}
	...
	public String getUserInfoById(String id) {
		...
		if (t != null) {
			// TODO
			School school = schoolService.selectByPrimaryKey(t.gettScSchoolId());
			SemesterVo semesterVo = semesterService.getCurrentSemester(t.gettScSchoolId());
			List<Clazz> clazzs = clazzService.selectTeacherClazzs(t.getId(), semesterVo.getId());
			String clazzName = "";
			...
			if (clazzName.length() > 1) {
				resultMap.put("clazzName", clazzName.substring(0, clazzName.length() - 1));
			} else {
			...
			if (p != null) {
				String schoolName = "";
				String clazzName = "";
				List<School> schools = schoolService.selectByParentPhone(p.getMobile());
				for (School s : schools) {
					schoolName += s.getName() + ",";
				}
				List<Student> students = studentService.findByParentId(p.getId());
				for (Student s : students) {
					clazzName += clazzService.clazzFind(s.gettScClassId()).getName() + ",";
				...
				resultMap.put("clazzName", clazzName.substring(0, clazzName.length() - 1));
				resultMap.put("schoolName", schoolName.substring(0, schoolName.length() - 1));
			} else {
				resultMap.put("success", false);
	...
	@ResponseBody
	public String resetPassword(HttpSession session, String userId, String adminPassword, String newPassword,
		...
		if (!NotEmptyString.isNotEmpty(new String[] { newPassword, repeatPassword })) {
	...
	/**
	 * 
	 * @dateTime 2016年6月14日下午7:03:48
	 * @author zjd
	 * @description 获取每个学校下各个班级家长的登陆和未登录人数,暑假可以删除，给小红用的而已，没有跟前端对接
	 * */
	@RequestMapping("/getUserLoginInfoForXiaohong")
	@ResponseBody
	public String getUserLoginInfoForXiaohong(String schoolId, HttpSession session) {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		resultMap.put("success", false);
		if (NotEmptyString.isNotNullString(schoolId)) {
			List<UserLoginInfoList> testList = new ArrayList<UserLoginInfoList>();
			List<UserLoginInfo> loginInfo = new ArrayList<UserLoginInfo>();
			
			List<Grade> gradeList = gradeService.getAllGrade(schoolId);
			
			for(Grade grade: gradeList) {
				List<Clazz> clazzList = clazzService.getClazzByGradeId(grade.getId());
				if(clazzList != null && clazzList.size() != 0) {
					for(Clazz clazz: clazzList) {
						UserLoginInfoList test = new UserLoginInfoList();
						int loginNumber = 0, notLoginNumber = 0;
						loginInfo = this.UserService.getUserLoginInfo(clazz.getId());
						for (int i = 0; i < loginInfo.size(); i++) {
							if(loginInfo.get(i).getLastLogin() == null) {
								notLoginNumber++;
							}else {
								loginNumber++;
							}
						}
						test.setClazz(clazz.getName());
						test.setLoginNumber(loginNumber);
						test.setNotLoginNumber(notLoginNumber);
						testList.add(test);
					}
				}
			}
			resultMap.put("登陆情况", testList);
			resultMap.put("success", true);
			return JSONUtil.toObject(resultMap).toString();
	...
	@RequestMapping("/getLoginCountOneClass")
	@ResponseBody
	public String getLoginCountOneClass(String classId, HttpSession httpSession) {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		
		int allMan = Integer.parseInt(UserService.getAllMan(classId));
		int loginMan = Integer.parseInt(UserService.getLoginMan(classId));
		int notLoginMan = allMan - loginMan;
		double loginPercentage = allMan != 0 ?
				(double) loginMan / allMan : 0;
		
		resultMap.put("allMan", allMan);
		resultMap.put("loginMan", loginMan);
		resultMap.put("notLoginMan", notLoginMan);
		resultMap.put("loginPercentage", new DecimalFormat("0.00").format(100 * loginPercentage) + "%");
		resultMap.put("success", true);
		return JSONUtil.toObject(resultMap).toString();
	}
	...
	public String getLoginCount(String schoolId, HttpSession httpSession) {
		...
		int schoolCountOfParent = Integer.parseInt(UserService.getLoginCountForSchool(schoolId));
		int notLoginCountOfParent = Integer.parseInt(UserService.getNotLoginCountForSchool(schoolId));
		...
		int schoolCountOfTeacher = Integer.parseInt(UserService.getLoginCountForSchoolTeacher(schoolId));
		int notLoginCountOfTeacher = Integer.parseInt(UserService.getNotLoginCountForSchoolTeacher(schoolId));
		...
		resultMap.put("schoolCountOfParent", schoolCountOfParent);
		resultMap.put("NotLoginCountOfParent", notLoginCountOfParent);
		resultMap.put("schoolCountOfTeacher", schoolCountOfTeacher);
		resultMap.put("NotLoginCountOfTeacher", notLoginCountOfTeacher);
		resultMap.put("loginCountOfTeacher", loginCountOfTeacher);
		resultMap.put("loginCountOfParent", loginCountOfParent);
		resultMap.put("loginParentPercentage", new DecimalFormat("0.00").format(100 * loginParentPercentage) + "%");
		resultMap.put("loginTeacherPercentage", new DecimalFormat("0.00").format(100 * loginTeacherPercentage) + "%");
		resultMap.put("success", true);
		return JSONUtil.toObject(resultMap).toString();
	}

	@RequestMapping("/showNewFeatures")
	@ResponseBody
	public String showNewFeatures(String userId) {
		HashMap<String, Object> resultMap = new HashMap<String, Object>();
		resultMap.put("success", false);
		if (NotEmptyString.isNotNullString(userId)) {
			try{
				ShowFeatures showFeatures = this.showFeaturesService.getUserReadStatus(userId);
				//如果该用户不在新特性的显示表中，将其添加到表内
				if(showFeatures == null){
					showFeatures = new ShowFeatures();
					showFeatures.setStatus(0);
					showFeatures.setUserId(userId);
				}
				if(this.showFeaturesService.insertNewUserToFeature(showFeatures)>0){
					resultMap.put("status", showFeatures.getStatus());
					resultMap.put("userId", showFeatures.getUserId());
				    resultMap.put("success", true);
				}
			}catch(Exception e) {
			}
	...
	public String setAlreadyRead(String userId, String status) {
		HashMap<String, Object> resultMap = new HashMap<String, Object>();
		resultMap.put("success", false);
		if (NotEmptyString.isNotEmpty(new String[] { userId, status })) {
			ShowFeatures showFeatures = this.showFeaturesService.getUserReadStatus(userId);
			showFeatures.setStatus(Integer.parseInt(status));
			if (this.showFeaturesService.udpateUserReadStatus(showFeatures) == 1)
				resultMap.put("success", true);
		}
		return JSONUtil.toObject(resultMap).toString();
	}
