	...
	init: function (application) {
		this.control({
			...
			'appraisemgr #identityCombo': {
				render: this.setDefaultIdentity
			},
      ...
      //导出评语模板
      "appraisemgr toolbar button#downloadTpl":{
           click: this.downloadTpl
      },
			...
			"appraisemgr #classCombo, appraisemgr #termCombo, appraisemgr #identityCombo": {
				change: function() {
					this.setIdentity();
				}
			},
			...
			'wordtpl': {
				//模块内评语太长，需要提示框
				itemmouseenter: function(view,record,mode,index,e) {
					this.tip = Ext.create('Ext.tip.ToolTip', {  
            target: view.el, //目标ele获取          
            delegate: view.itemSelector,   //每个元素要不一样时候有用
            trackMouse: true,          //提示框
            renderTo: Ext.getBody(),   //在body上render
            listeners: {               
              beforeshow: function(tip) {  //show之前将值set入
                tip.update(view.getRecord(tip.triggerElement).get('word'));  
          		}
            }  
          });
        }
  ...
  getSubjectWordTpl: function (combo) {
	...
	doSaveAppraise: function (btn) {
		...
		var appraiseList = me.getAppraiseList(),
			...
			semester = appraiseList.down('#termCombo'),
			identityCombo = appraiseList.down('#identityCombo');

		params.semester = semester.getValue();
...
//导出评语模板
  downloadTpl:function(){
  	var url = "/school/appraiseSubject/downloadAppraiseExcel"; 
...
//  设置默认评价身份
    setDefaultIdentity:function(combo){
        var store = combo.getStore();
        store.reload({
        	callback:function(){
        		var record = combo.getStore().getAt(0);	
        		record && combo.setValue(record.get("type"));
        	}
        })  
    },
  // 设置评价身份
	setIdentity: function () {
		...
		identityCombo.getStore().reload({
			params: {
				classId: classCombo.getValue(),
				semesterId: termCombo.getValue()
			},
			callback: function() {
				//因为身份设置要学期参数以及班级参数，所以在render时候可能会尚未读取到，所以在这里写上保险
				if(!identityCombo.getValue()) {
