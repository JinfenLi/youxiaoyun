...
public class ExamServiceImpl  extends BaseServiceImpl<Exam> implements ExamService{
	...
	@Resource
	private ExamTempletMapper examTemletMapper;
	@Resource
	...
	public List<ExamInfo> selectExamRecord(Map<String, Object> map) {
		...
		for(int i = 0; i < examInfos.size(); i++) {
			...
			examInfos.get(i).setExamTemplet(examTemletMapper.selectByPrimaryKey(examInfos.get(i).getTempletId()));
		}
		return examInfos;
	}
	...
	public List<ExamTemplet> getAllExamTemplet() {
			return examTemletMapper.selectAll();
	}

	@Override
	public boolean addExamTemplet(ExamTemplet examTemplet) {
		try {
			examTemplet.setId(UUIDUtil.getUUID());
			examTemplet.setCreateTime(new Date());
			return examTemletMapper.insertSelective(examTemplet) > 0 ? true : false;
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		}
