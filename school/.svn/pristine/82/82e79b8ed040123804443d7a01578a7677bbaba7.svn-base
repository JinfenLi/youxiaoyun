...
public class CurriculaMapperImpl extends BaseDaoImpl<Curricula> implements
	...
	@Override
	public List<CurriculaInfoVo> getCurriculaInfo(String semesterId,
			String classId) {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("semesterId", semesterId);
			map.put("classId", classId);
			return template.selectList(getStatement("selectCurriculaInfo"),map);
	}

	@Override
	public Curricula getCurriculaByExamId(String examId) {
		return template.selectOne(getStatement("getCurriculaByExamId"), examId);
	}
