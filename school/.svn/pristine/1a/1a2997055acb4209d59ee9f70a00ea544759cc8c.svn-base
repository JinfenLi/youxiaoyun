...
public class ClazzAlbumController {
	...
	/**
	 * @dateTime 2016年8月4日下午12:36:16
	 * @author zjd
	 * @description 更新相册封面
	 */
	@RequestMapping("/updateAlbumCover")
	@ResponseBody
	public String updateAlbumCover(String coverPath, String albumId) {
		Map<String,Object> resultMap = new HashMap<String, Object>();
		AlbumInfo albumInfo = new AlbumInfo();
		albumInfo.setCoverPath(coverPath);
	...
	/**
	 * @dateTime 2016年9月18日上午11:01:52
	 * @author zjd
	 * @description 审核未通过的照片
	 */
	@RequestMapping("checkTip")
	@ResponseBody
	public String checkTip(String clazzIds, HttpSession session, Pager pager, Integer limit, Integer start) {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		UserInfo userInfo = (UserInfo) session.getAttribute("currentUser");
		resultMap.put("success", false);
		if (clazzIds == null || "".equals(clazzIds)) {
			resultMap.put("msg", "很抱歉您没有相应权限查看");
			return JSONUtil.toObject(resultMap).toString();
		}
		String[] clazzId = clazzIds.split(",");
		for(String cId:clazzId) {
			AlbumInfo info = new AlbumInfo();
			info.settMId(cId);
			info.setType("未审核");
			List<AlbumInfo> albums = albumService.albumFindByType(info).getInfoResult(); // 查询未审核相册，一定存在
			if (userInfo.getUser_type().ordinal() == 1) { // 如果是教师返回相册和未审核的照片
				...
				photo.settMId(albums.get(0).getId());
				photo.setPager(pager);
				if(photoService.selectCount(albums.get(0).getId()) != 0) {
					resultMap.put("success", true);
					return JSONUtil.toObject(resultMap).toString();
				}
			}
