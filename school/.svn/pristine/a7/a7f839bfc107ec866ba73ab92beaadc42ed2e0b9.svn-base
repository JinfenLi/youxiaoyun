...
public class ClazzScoreController {
	...
	public String getExcellentRate(String clazzId, String examId, String termId) {
		...
		List<Exam>  list_exam = examService.getExamByCurriculaIdAndTermId(curricula.getId(), termId, clazzId);
		Map<String,Object> resultMap = new HashMap<>();
		List<String> exam_name = new ArrayList<String>();
		List<Integer> exam_excellentRate = new ArrayList<Integer>();
		resultMap.put("success", false);
		for(Exam exam: list_exam) {
			Map<String, Object> map = getExcellentRateOne(clazzId, exam.getId());
			exam_name.add(map.get("name").toString());
			exam_excellentRate.add(Integer.parseInt(map.get("value").toString()));
		}
		if(list_exam != null) {
			resultMap.put("name", exam_name);
			resultMap.put("value", exam_excellentRate);
			resultMap.put("success", true);
		}
		return JSONUtil.toObject(resultMap).toString();
	}
	
	private Map<String,Object> getExcellentRateOne(String clazzId, String examId) {
		ScoreInfoResult scoreInfoResult = scoreService.getScoreByExamIdAndClazzId(examId, clazzId);
		Map<String,Object> resultMap = new HashMap<>();
		int excellentNumber = 0;
		if(scoreInfoResult != null) {
			List<ScoreInfo> list = scoreInfoResult.getResult();
			for(ScoreInfo scoreInfo: list) {
				if(Float.parseFloat(scoreInfo.getScore()) > 80) {
					excellentNumber++;
				}
			}
			resultMap.put("name", scoreInfoResult.getResult().get(0).getExamName());
			resultMap.put("value", (int)(excellentNumber / (list.size() + 0.0) * 100));
		}
		return resultMap;
	}
	...
	public String getScoreTrenderForStudent(String examId, String clazzId, String termId, String studentId){
		...
		if(NotEmptyString.isNotEmpty(new String[] {curricula.getId(), termId, studentId, clazzId})){
			...
			Collections.sort(list);//按照考试时间排序
			for(Exam exam: list){
