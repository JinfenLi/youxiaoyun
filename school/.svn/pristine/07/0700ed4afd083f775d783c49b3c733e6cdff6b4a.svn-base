	...
	public boolean saveScore(String fileName, String realPath, String examId) {
		...
		for (ScoreInfo info : scoreInfos) {
			...
			if (result.size() > 0) { // 如果存在加入数组方便后续推送操作
				...
				System.out.println("否则学生姓名和学号不在该系统中则上传失败");
				return false;
			}
...
/*	*//**
	...
	* @throws
	...
	public List<Score> getScoreByClazzIdAndCurriculaIdAndTermId(String clazzId, String curriculaId, String termId) {
		List<Score> list = scoreMapper.getScoreByClazzIdAndCurriculaIdAndTermId(clazzId, curriculaId, termId);
		return list;
	}

	@Override
	public String getExamNameByExamId(String examId) {
		return scoreMapper.getExamNameByExamId(examId);
	}

	@Override
	public String getMaxScore(String examId) {
		return scoreMapper.maxScore(examId);
	}

	@Override
	public String getMinScore(String examId) {
		return scoreMapper.minScore(examId);
	}
}
