	...
	*/
	...
	editExam: function(button) {
		...
		var record = button.up("grid").getSelectionModel().getSelection(), 
			win = {};
		var targetRecord = record[0].get("examTemplet");
		var examId = "";
		if(targetRecord && targetRecord.id) {
			examId = targetRecord.id;
		}
		//如果没有选择需要编辑的考试，则提示用户
		if(!record[0]) {
	...
	getAllExamType: function(component) {

		var examCombobox = component.down("combobox"),
			examStore = examCombobox.getStore(),
			examId = component.examId;
		console.log(examId);
		examStore.reload({
			callback: function(records, options, success) {
				!examId || examCombobox.setValue(examId);		
			}
		});
