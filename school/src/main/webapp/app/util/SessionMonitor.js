 ...
 * logged out.
 */
Ext.define('School.util.SessionMonitor', {
	singleton: true,
	requires: [
		'Ext.window.Window'
	],
	...
	lastActive: null,
	maxInactive: 1000 * 60 * 20,  // 20 minutes of inactivity allowed; set it to 1 for testing.
	remaining: 0,
	ui: Ext.getBody(),
	...
	window: Ext.create('Ext.window.Window', {
		...
		buttons: [{
			text: '继续登陆',
			handler: function () {
				Ext.TaskManager.stop(School.util.SessionMonitor.countDownTask);
				School.util.SessionMonitor.window.hide();
				School.util.SessionMonitor.start();
				// 向后台发送请求，维持session
				Ext.create("School.store.school.TeacherMgr").reload();
			}
		}, {
			text: '退出登陆',
			action: 'logout',
			handler: function () {
				...
				Ext.ComponentQuery.query('#logout')[0].fireEvent('click',
						Ext.ComponentQuery.query('#logout')[0]);
			}
		}]
	}),
	 ...
	 */
	countDown: function () {
		this.window.down('label').update('自动退出剩余时间为： ' + this.remaining + ' 秒' + ((this.remaining == 1) ? '.' : 's.'));
