...
Ext.define("School.controller.school.GradeMgr", {
	...
	createEditWin: function(button) {
		...
		win.down("form").loadRecord(record[0]);
		 win.down("form").queryById("gradeName").setReadOnly(true);
	},
    ...
    upGrade:function(button){
    	School.util.Util.confirm("全校年级升级，修改后不可返回当前状态",function(buttonId){
    		if(buttonId !=="yes"){//不确定退出操作
    			return;
    		}
    		var url =zyHost + "/school/sch/levelUpGradeBySchoolId.action",
    		    params = {
    		    	tScSchoolId :zy_schoolId
    		    },
    		    store =  button.up("gridpanel").getStore();
    		...
    		Ext.Ajax.request({
    			...
    			success: function(conn, response, options, eOpts) {
    				var result = School.util.Util.decodeJSON(conn.responseText);
    				    ...
    				    if (result.success) {
    				    	School.util.Util.showSuccessMsg("全校年级升级成功！");
    				        store.reload();
    				    }
    			},
