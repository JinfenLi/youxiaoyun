...
public class ArticleController {
	...
	public String getArticleById(HttpSession session,HttpServletRequest request, String id) {
		...
		List<RichTextInfo> infos = schoolArticleService.getArticleById(id).getInfoResult();
		if(infos.isEmpty()) {
			resultMap.put("success", false);
		} else {
			RichTextInfo info = infos.get(0);
			info.setUrl(request.getContextPath()+"/article/getArticle.action?articleId=" + id);
			RichTextInfoWithFile article = RichTextInfoWithFile.changeToRichTextInfoWithFile(info);
			resultMap.put("success", true);
