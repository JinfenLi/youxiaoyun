...
public class GetLeaveProcess implements LeaveProcess {
	...
	public boolean doProcess(LeaveProcessContext context) {
		...
		if(context.getSession().getAttribute("status") != null){
			status = (Integer)context.getSession().getAttribute("status");
		}
		...
		case 1: // 老师类型根据班级id获取请假申请记录
			...
			totalCount = leaveMapper.selectCountByClazzIdAndStatus(map);
			break;
		case 2:// 家长类型根据学生id获取请假申请记录
			...
			totalCount = leaveMapper.selectCountByStudentIdAndStatus(map);
			break;
		default:// 非法用户
