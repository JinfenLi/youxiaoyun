...
public class ClazzPhotoServiceImpl implements ClazzPhotoService {
	...
	private List<ClazzPhotoProcess> clazzPhotoUpdateSortAndDescriptionProcesses;
	...
	@Override
	public PhotoInfoResult updatePhotoSortAndDescription(PhotoInfo info) {
		ClazzPhotoProcessContext context = new ClazzPhotoProcessContext();
		PhotoInfoResult result = new PhotoInfoResult();
		context.setInfo(info);
		context.setResult(result);
		for (ClazzPhotoProcess process : clazzPhotoUpdateSortAndDescriptionProcesses) {
			if (!process.doProcess(context)) {
				break;
			}
		}
	...
	public List<ClazzPhotoProcess> getClazzPhotoUpdateSortAndDescriptionProcesses() {
		return clazzPhotoUpdateSortAndDescriptionProcesses;
	}
	
	public void setClazzPhotoUpdateSortAndDescriptionProcesses(
			List<ClazzPhotoProcess> clazzPhotoUpdateSortAndDescriptionProcesses) {
		this.clazzPhotoUpdateSortAndDescriptionProcesses = clazzPhotoUpdateSortAndDescriptionProcesses;
	}
